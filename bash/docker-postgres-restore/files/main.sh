...
usage() {
  printf '%s\n' \
    ...
    "  $SCRIPT_NAME [options]" \
    "" \
    "Restore a PostgreSQL .sql or .sql.gz archive into a running Docker container." \
    "" \
    "Options:" \
    "  --container NAME         Target PostgreSQL container (default: $CONTAINER_NAME_DEFAULT)" \
    "  --archive PATH           Path to a .sql or .sql.gz archive (default: $ARCHIVE_PATH_DEFAULT)" \
    "  --user USER              PostgreSQL user for psql (default: $POSTGRES_USER_DEFAULT)" \
    "  --database NAME          Target database or 'all' (default: $POSTGRES_DATABASE_DEFAULT)" \
    ...
    "  $SCRIPT_NAME --container postgres --archive /backups/postgres/app.sql.gz --database appdb" \
    "  $SCRIPT_NAME --archive /backups/postgres/all.sql.gz --database all --yes" \
    "  $SCRIPT_NAME --archive /backups/postgres/app.sql --database appdb --drop-recreate --dry-run" \
    "" \
    "Notes:" \
...
preflight() {
  ...
  [[ -n "$POSTGRES_DATABASE" ]] || fail "Postgres database must not be empty."
  ...
  docker exec "$CONTAINER_NAME" sh -c 'command -v psql >/dev/null 2>&1' \
    || fail "psql is not available inside container: $CONTAINER_NAME"
  ...
  if bool_is_true "$DROP_RECREATE" && [[ "${POSTGRES_DATABASE,,}" == "all" ]]; then
    fail "--drop-recreate cannot be used when --database is 'all'."
  fi
}

run_drop_recreate() {
  local db_ident db_literal sql
  db_ident="$(sql_ident "$POSTGRES_DATABASE")"
  db_literal="$(sql_literal "$POSTGRES_DATABASE")"
  sql="$(
    printf '%s\n' \
      "SELECT pg_terminate_backend(pid)" \
      "FROM pg_stat_activity" \
      "WHERE datname = '$db_literal'" \
  ...
  if bool_is_true "$DRY_RUN"; then
    log "Dry run: would terminate active sessions and recreate database '$POSTGRES_DATABASE'."
    return 0
  fi

  log "Dropping and recreating database '$POSTGRES_DATABASE'."
  printf '%s\n' "$sql" | docker_psql postgres
}
...
run_restore() {
  ...
  if bool_is_true "$DRY_RUN"; then
    if [[ "$ARCHIVE_PATH" == *.sql.gz ]]; then
      log "Dry run: would stream gzip archive $(shell_quote "$ARCHIVE_PATH") into psql database '$target_db'."
    else
      log "Dry run: would stream SQL archive $(shell_quote "$ARCHIVE_PATH") into psql database '$target_db'."
    fi
    return 0
  fi

  log "Restoring archive into database target '$target_db'."
  stream_archive | docker_psql "$target_db"
}
